...
function jdjr() {
    ...
    ids = 'rl_top_searchiv_close';
    let po = func.item_is_load('id', ids, clsName);
    if (po != false) {
        func.item_click(po);
    }
    //点击信用
    ...
    texts = '已领取';
    // 判断是否已领券
    if (func.item_is_load('text', texts, clsName)) {
        console.log('今日已领券');
    } else {
...
function youchu() {
    ...
    ids = 'user_myrp';
    texts = '我的额度'; 
    func.wait_item_load('text+id', texts + '+' + ids);
    // 检查每日签到
    clsName = 'android.widget.ImageView';
...
function gonghang() {
    ...
    func.wait_load_click('text', texts, clsName);
    texts = '已签到';
    if (func.wait_item_load('text', texts, clsName)) {
        console.log('已签到');
    }
}
// ---------------------工银e生活签到---------------------------
