...
function launchApp() {
    ...
    do {
        log("尝试启动今日头条极速版")
        isLauchApp = launchPackage("com.ss.android.article.lite")
        sleep(_delayTime * 2)
    } while (!isLauchApp)
    log("今日头条极速版已启动")
    return true
...
function signin() {
    log("签到")
    let taskTag = text("任务").findOne(_delayTime)
    if (taskTag) {
        taskTag.parent().click()

        let signinBtn = text("签到").findOne(_delayTime)
        if (signinBtn) {
            signinBtn.click()
...
function watchAD() {
    ...
    if (activityBtn) {
        ...
        let adTag = text("限时任务赚金币").findOne(_delayTime)
        if (adTag) {
...
function ReadArticleThree() {
    log("认真阅读3篇文章")
    let taskTag = text("阅读赚金币").findOne()
    if (taskTag) {
        taskTag.parent().click()

        btnTag = text("认真阅读3篇文章").findOne()
        if (btnTag) {
            let progress = btnTag.parent().parent().child(1).child(2)
            log(progress.text())
            if (progress.text().match("3/3")) {
                log("已经完成认真阅读3篇文章")
            } else {
                btnTag.parent().parent().click()
                sleep(_delayTime)

                click(990, 1840)
                sleep(_delayTime)

                let newsBtns = id("com.ss.android.article.lite:id/a16").find()
                for (let i = 2; i < newsBtns.length && i < 3; i++) {
                    let news = newsBtns[i].parent()
                    if(news){
                        news.click()
                        sleep(5000)
                        back()
                ...
                // 看广告
                // let adTextTag = text("看完视频再领").findOne(_delayTime)
                // if (adTextTag) {
                //     adTextTag.parent().click()
                //     sleep(_adTime)
                //     click(1300, 125)
...
function openBox() {
    ...
    if (taskTag) {
        ...
        let btnTag = text("treasure-box-enable-1.da338c08").findOne(_delayTime)
        //log(btnTag)
        if (btnTag) {
            btnTag.parent().click()
...
function eatSubsidy() {
    ...
    if (activityBtn) {
        ...
        let textTag = text("吃饭补贴").findOne(_delayTime)
        if (textTag) {
...
function readNovels() {
    ...
    if (activityBtn) {
        ...
        let readTag = text("看小说赚金币").findOne(_delayTime)
        if (readTag) {
